...
const Content = ({ text, setText, post, me, handleCreateComment, handleDeleteComment, handleLike, handleUnlike, setOpenMask, headShotURLs, navigate }) => (
    <MyContent>
        <div>
            <Typography sx={{ fontSize: 25, fontWeight: 'bold' }}>{post.author}</Typography>
        </div>
        <div className='post-body'>
            {post.body.map((body, idx) => {
                if(body==="") return <br key={idx}/>
                else return <Paragraph key={idx} style={{fontSize: 20, marginBottom: 0, whiteSpace: 'pre-wrap' }}>{body}</Paragraph>
            })}
        </div>
        ...
        <>
            <PageHeader
                ...
                extra={post.author===me? [
                    ...
                    <Popconfirm key="1" placement="topRight" title="Delete the post？" onConfirm={() => handleDeletePost(post.postId)} okText="Yes" cancelText="No" onCancel={() => setOpenMask(false)}>
